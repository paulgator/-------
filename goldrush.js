...
function getValuableTarget(a,s,e) {
    ...
    var t=0;
        ...
        v=Math.pow(a[i].bountyGold,2)/Math.pow(getDistance(s.pos,a[i].pos),2);
            if (v>mv && getDistance(a[i].pos,s.pos)<getDistance(a[i].pos,e.pos)-5) {
                mv=v;
                t=a[i];
            }
        }    
...
if (target!==0 && items.length>0 && (jumptime>0 || (longJumpValue[1]<=12 && longJumpValue[1]<150-this.gold)|| target.bountyGold>3)) {
    this.move(target.pos);
    } else if ((target!==0 && items.length>0 && jumptime===0 && (longJumpValue[1]>12 || longJumpValue>=150-this.gold))) {
