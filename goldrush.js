...
a[i].pos.x<b.x+70 && a[i].pos.y>b.y-5 && a[i].pos.y<b.y+5) {
            right+=a[i].bountyGold;
            } else if (a[i].pos.x<b.x && 

a[i].pos.x>b.x-70 && a[i].pos.y>b.y-5 && a[i].pos.y<b.y+5) {
                left+=a[i].bountyGold;
                } else if (a[i].pos.x<b.x+5 && 

a[i].pos.x>b.x-5 && a[i].pos.y>b.y && a[i].pos.y<b.y+70) {
                    up+=a[i].bountyGold;
                    } else if (a[i].pos.x<b.x+5 
...
(Math.max(up,down,left,right)==up) {
        return ["up",up];
...
(Math.max(up,down,left,right)==left) {
                    return ["left",left];
                    } else {
                        return [];
...
getDistance(a[i].pos,s.pos)<getDistance(a[i].pos,e.pos)-5) {
                mv=v;
                t=a[i];
            }
        }    
...
getDistance(a[i].pos,s.pos)<getDistance(a[i].pos,e.pos)) {
                mv=v;
                t=a[i];
            }
        }    
...
if (target!==0 && items.length>0 && (jumptime>0 || 

(longJumpValue[1]<=12 && longJumpValue[1]<150-this.gold)|| target.bountyGold>3)) {
    this.move(target.pos);
    } else if ((target!==0 && 

items.length>0 && jumptime===0 && (longJumpValue[1]>12 || longJumpValue>=150-this.gold))) {
        switch (longJumpValue[0]) {
         case "up": 

this.jumpTo({x:this.pos.x,y:this.pos.y+70}); break;
         case "down": this.jumpTo({x:this.pos.x,y:this.pos.y-70}); break;
         case 

"right": this.jumpTo({x:this.pos.x+70,y:this.pos.y}); break;
